...
SpikeCosim::SpikeCosim(uint32_t start_pc, uint32_t start_mtvec,
  ...
  processor =
      std::make_unique<processor_t>("RV32IMC", "MU", DEFAULT_VARCH, this, 0,
                                    false, log_file, nullptr, secure_ibex,
                                    icache_en);

  // TODO: Uncomment when rebasing on up to date spike
...
bool SpikeCosim::check_mem_access(bool store, uint32_t addr, size_t len,
  ...
  if (store || !top_pending_access_info.error) {
    // Combine bytes into signal word
