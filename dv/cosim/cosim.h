...
struct DSideAccessInfo {
  ...
  // aligned to the address. For example an 8-bit store of 0xff to 0x2 has
  // `data` as 0x00ff0000 and `be` as 0b0100.
  uint32_t data;
  uint32_t addr;
  uint32_t be;

  // set if an error response is seen to the transaction.
  bool error;
...
class Cosim {
 public:
  ...
  // NMI is asserted following the `mret` (regardless of whether it has been
  // deasserted or not whilst the first NMI is being handled) an new NMI is
  // taken.
  //
